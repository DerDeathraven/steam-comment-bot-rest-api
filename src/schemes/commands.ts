...
export const commands: Array<CommandDescription> = [
  ...
  {
    name: "comment",
    description: "Requests comments from all available bot accounts for a profile, group or sharedfile",
    args: [
      {
        name: "amount",
      ...
      {
        name: "ID",
        description: "The link, steamID64 or vanity of the profile, group or sharedfile to comment on (Owner only)",
        type: "string",
      },
      {
        name: "custom quotes",
        description: "Array of strings to use as quotes in this comment request instead of the default quotes.txt set (Owner only)",
        type: "string"
      }
    ],
  },
  ...
  {
    name: "downvote",
    description: "Downvotes a sharedfile with all bot accounts that haven't yet voted on that item. Requires unlimited accounts! (Owner only)",
    args: [
      {
        name: "amount",
  ...
  {
    name: "unfavorite",
    description: "Unfavorizes a sharedfile with all bot accounts that have favorized that item. (Owners only)",
    args: [
      {
        name: "amount",
  ...
  {
    name: "resetcooldown",
    description: "Clear your, the ID's or the comment cooldown of all bot accounts (global). (Owner only)",
    args: [
      {
        name: "ID or \"global\"",
  ...
  {
    name: "addfriend",
    description: "Adds the ID with all bot accounts. Requires unlimited accounts! (Owner only)",
    args: [
      {
        name: "ID",
  ...
  {
    name: "unblock",
    description: "Unblocks a user with all bot accounts on Steam. Note: The user can still get ignored by Steam for a while. (Owner only)",
    args: [
      {
        name: "ID",
  ...
  {
    name: "reload",
    description: "Reloads all commands and plugins without needing to restart. Please only use it for testing/development. (Owner only)",
    args: []
  },
  {
    name: "restart",
    description: "Restarts the bot and checks for available updates. (Owner only)",
    args: []
  },
  {
    name: "stop",
  ...
  {
    name: "update",
    description: "Checks for an available update and installs it if automatic updates are enabled and no requests are active. 'true' forces an update. Blocks new requests if it currently waits for one to be completed. (Owner only)",
    args: [
      {
        name: "\"true\"",
  ...
  {
    name: "eval",
    description: "Disabled by default, needs to be toggled on with `enableevalcmd` in config.json. **Warning!** This will run any javascript code that was provided. It is strongly advised to leave this feature off unless you know exactly what this means! If you have multiple owners configured they can also run code on **your** machine! (Owner only)",
    args: [
      {
